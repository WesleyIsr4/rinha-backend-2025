...
class ConsistencyService {
  ...
  async verifyPaymentConsistency(
    correlationId,
    amount,
    processorType,
    requestedAt
    ...
    const duplicateCheck = await this.verifyNoDuplicateCorrelationId(
      correlationId
    );
  ...
  verifyAmountFormat(amount) {
    ...
    const hasCorrectPrecision =
      Number.isFinite(amount) && (amount * 100) % 1 === 0; // Max 2 decimal places
  ...
  verifySummaryStructure(summary) {
    ...
    const hasDefaultRequests =
      hasDefault && typeof summary.default.totalRequests === "number";
    const hasDefaultAmount =
      hasDefault && typeof summary.default.totalAmount === "number";
    const hasFallbackRequests =
      hasFallback && typeof summary.fallback.totalRequests === "number";
    const hasFallbackAmount =
      hasFallback && typeof summary.fallback.totalAmount === "number";
  ...
  getConsistencyStats() {
    ...
    const consistencyLogs =
      this.auditService.getAuditLogsByEvent("CONSISTENCY_CHECK");
